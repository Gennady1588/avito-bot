...
# ----------------- ОБРАБОТЧИКИ КЛИЕНТА -----------------
# (Большая часть осталась прежней)
...
def proc_dep(m):
    ...
    # 🔥 ИЗМЕНЕНИЕ ЛОГА: УДАЛЕНЫ КРУГЛЫЕ СКОБКИ ВОКРУГ ID
...
def extract_client_id_from_text(text):
    """
    Находит ID клиента в тексте сообщения, используя новое регулярное выражение.
    Ищем: 'ID: 7579757892'
    """
    if not text:
        return None
    # 🔥 НОВОЕ АГРЕССИВНОЕ РЕГУЛЯРНОЕ ВЫРАЖЕНИЕ: Ищем "ID: " за которым следуют 8-12 цифр
...
def admin_action_handler(m):
    ...
    # ПЫТАЕМСЯ НАЙТИ ID В ТЕКСТЕ, НА КОТОРЫЙ ОТВЕЧАЕМ
    client_chat_id = extract_client_id_from_text(replied_message_text)
    
    if not client_chat_id:
        # Если ID не найден, возвращаем ошибку с указанием, что теперь нет скобок
