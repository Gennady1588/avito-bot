...
def process_deposit_amount(message):
    ...
    # Обработка нажатия кнопки "Отмена/Назад"
    ...
    if not message.text:
        ...
        # Передаем None, так как старое сообщение уже удалено
    ...
    payment_instruction = (
        ...
        # ⚠️ ГАРАНТИЯ, ЧТО НОМЕР КАРТЫ ЗДЕСЬ
        f"💳 **`{YOUR_CARD_NUMBER}`**\n\n" 
    ...
    # --- УВЕДОМЛЕНИЕ АДМИНИСТРАТОРА (С НОВЫМ ФОРМАТОМ ДЛЯ НАДЕЖНОГО ПАРСИНГА) ---
    ...
    try:
        # УДАЛЕНИЕ: bot.send_message(chat_id, ...) - тут была ошибка, что удалялось не то сообщение
        bot.send_message(
            OWNER_ID, 
            deposit_summary_for_admin, 
            parse_mode='Markdown'
        ...
        # Ошибка отправки админу - игнорируем, но печатаем (если есть логи)
        print(f"Error sending admin deposit notification for chat {chat_id}: {e}") 
    
    # ГАРАНТИРОВАННАЯ ОТПРАВКА НОМЕРА КАРТЫ КЛИЕНТУ (С БЛОКОМ TRY/EXCEPT)
    try:
        bot.send_message(
            chat_id, 
            payment_instruction,
        ...
        fallback_text = (
            ...
            f"Номер карты для перевода: {YOUR_CARD_NUMBER}. "
            f"Свяжитесь с менеджером @{MANAGER_USERNAME}."
        )
        
        # Отправляем максимально простой текст
        bot.send_message(
            chat_id, 
            fallback_text
        )
...
def client_msg(m):
    ...
    # ⚠️ Убедимся, что это не сообщение, отправленное степ-хэндлером, которое не должно быть переслано
    # (Хотя мы уже удаляем сообщения в степ-хэндлерах, это дополнительная защита)
    
    if m.content_type != 'text':
         # Игнорируем фото, видео и т.д., если это не в рамках step_handler
         return
