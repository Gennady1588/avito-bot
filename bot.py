...
def find_client_id(message):
    ...
    # 2. Если не найден, поиск в цитируемом сообщении (reply_to_message)
...
# 1. ОБРАБОТЧИК ДЛЯ НАЧИСЛЕНИЯ БАЛАНСА (через команду /add_balance)
...
def add_balance_command(m):
    try:
        # Извлекаем сумму из текста команды (например, из /add_balance 400)
        amount = int(m.text.split()[1])
    except:
        return bot.reply_to(m, "❌ ОШИБКА: Используйте формат /add_balance 400 (без фигурных скобок).")

    # Ищем ID клиента в сообщении, на которое отвечаем (m.reply_to_message)
    ...
    if client_id_match:
        ...
        # НОВЫЙ ТЕКСТ ОШИБКИ
        bot.reply_to(m, "❌ ID клиента не найден в лог-сообщении, на которое вы отвечаете. Начисление отменено.")

# 2. ОБРАБОТЧИК ДЛЯ ПРОСТОГО ТЕКСТОВОГО ОТВЕТА (любой другой текст)
# Этот обработчик не должен срабатывать, если вы вводите команду /add_balance
...
def admin_reply(m):
    ...
    # Находим ID клиента, используя надежную функцию
    ...
    if not client_chat_id:
         # ЭТОТ ТЕКСТ ОШИБКИ БУДЕТ СРАБАТЫВАТЬ ТОЛЬКО ДЛЯ ОБЫЧНОГО ТЕКСТА
         bot.reply_to(m, "❌ КРИТИЧЕСКАЯ ОШИБКА: ID клиента не найден. Убедитесь, что вы отвечаете на лог-запрос.")
         return
